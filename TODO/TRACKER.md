...
|---|---|---|---|---|
| IN1 | Éditeur « VSCodium » dans l'UAC + URLs | `vscode/build/win32/code.iss:11-14` (`AppPublisher=VSCodium`, URLs `vscodium.com`) + exe principal sans `CompanyName` (`gulpfile.vscode.ts:542` ne patche que les deps) + re-stamp `prepare_vscode.sh:360-361` | **faible** (swaps + stop re-stamp + 1 rcedit exe) | ⬜ |
...
| IN3 | Logo VSCodium dans l'assistant d'install | `code.iss:23-24` → `vscode/resources/win32/inno-big/small-*.bmp` (VSCodium) ; `SetupIconFile`=code.ico **déjà Arclen ✅** ; source `icons/build_icons.sh:40,46` | **élevé** (design : créer les .bmp wizard Arclen multi-DPI) | ⬜ |
