# Arclen — Carte des surfaces (tracker de dé-scaring)

> **Tracker** (matrice statut + décisions + historique). Le **backlog actionnable** `- [ ]` lisible par `todo_scan.py` vit dans [`TODO.md`](TODO.md) ; ce fichier garde le « pourquoi » et l'historique des décisions.
> Source des décisions : brainstorm 2026-05-29 (voir `chantier-arclen.md` pour la vision).
...
| A5 | Extensions | HIDE icône + verrou marketplace (gallery retirée) + raccourci off | patch + prepare_vscode.sh | faible-moyen | 1 | ✅ fait — gallery retirée (search « python » = 0), icône cachée, **et raccourci Ctrl+Shift+X retiré** (`arclen-extensions-no-reveal`) → ne ré-révèle plus l'icône. Commande viewlet intacte (palette/API). ⚠️ **RÉOUVERT 2026-06-01 (décision user — usage perso, réversible)** : marketplace **Open VSX rétabli** (`prepare_vscode.sh` `extensionsGallery` décommenté), **icône ré-affichée** (`workbench.view.extensions` retiré du set caché d'`arclen-clean-activity-bar`), `extensions.autoUpdate/autoCheckUpdates:true` + `ignoreRecommendations:false`. **Résidu** : Ctrl+Shift+X reste off (`arclen-extensions-no-reveal` non touché) → icône cliquable mais pas le raccourci. À re-verrouiller pour le produit final. Voir lot #14. |
...
| G3 | Notifs update / extensions | setting (configurationDefaults) | ✅ fait (update.mode none) — ⚠️ **autoUpdate/autoCheckUpdates remis ON + ignoreRecommendations OFF (2026-06-01, réouverture extensions A5, réversible)** |
...
## 9. Installeur & métadonnées packaging — ⬜ BACKLOG (avant toute distribution externe)

> Découvert 2026-06-01 (user). L'**app** est brandée Arclen, mais la **couche installeur Inno Setup + le publisher Windows** fuient encore « VSCodium ». Le pipeline branding (`branding.sh`→`apply_branding.sh`) ne couvre PAS ces champs ; pire, `prepare_vscode.sh:360-361` **re-tamponne** VSCodium (`Microsoft Corporation`→`VSCodium`, `code.visualstudio.com`→`vscodium.com`) dans `code.iss` à chaque build. **Ne bloque PAS l'usage perso** (portable déjà brandé) → à faire **avant de distribuer l'installeur**. Fix propre = via `branding.sh`/`apply_branding.sh`, pas en hardcodant.

> **✅ FAIT 2026-06-04 (installeur + electron.ts)** : les seds de `prepare_vscode.sh` qui re-tamponnaient « VSCodium » dans `code.iss` (publisher, URLs, **`OutputBaseFilename`→`ArclenSetup`**), `electron.ts` (CompanyName + copyright `.exe` — IN4), le manifest serveur (IN5) et `package.json` (IN7) utilisent maintenant `${ORG_NAME}/${APP_NAME}/${GH_REPO_PATH}` — vars exportées par `dev/build.sh`, donc **pilotées par `branding.sh`, pas hardcodées** (approche plus simple que « étendre apply_branding.sh » du plan IN0). **IN3 = non-issue** : les BMP du wizard montrent déjà le logo corail Arclen (vérifié visuellement). `bash -n` + `jq` OK. Bloc Linux laissé (dead code sur Windows x64). **Reste** : IN2 (licence — décision légale user) + vérif métadonnées `.exe`/nom installeur sur build `-s` packagé.

| # | Fuite | Origine (file:line) | Coût | Statut |
|---|---|---|---|---|
| IN1 | Éditeur « VSCodium » dans l'UAC + URLs | `vscode/build/win32/code.iss:11-14` (`AppPublisher=VSCodium`, URLs `vscodium.com`) + exe principal sans `CompanyName` (`gulpfile.vscode.ts:542` ne patche que les deps) + re-stamp `prepare_vscode.sh:360-361` | **faible** (swaps + stop re-stamp) | ✅ **2026-06-04** (publisher/URLs/OutputBaseFilename + CompanyName/copyright via electron.ts) |
| IN2 | Licence VSCodium/MS affichée par l'installeur | `code.iss:1-5` macro → `vscode/LICENSE.rtf` (MIT VSCodium+Microsoft) | **moyen** — ⚠️ légal : fork MIT → on DOIT garder les mentions copyright MS+VSCodium ; on peut re-présenter une licence aux couleurs Arclen qui les inclut, **pas** les supprimer. **Décision validée (Adrian, 2026-06-04)** : garder les attributions MS+VSCodium, re-skin juste `LICENSE.rtf` au nom/couleurs Arclen. | ⬜ impl (décision prise ; reste re-skin `LICENSE.rtf` au packaging) |
| IN3 | Logo VSCodium dans l'assistant d'install | `code.iss:23-24` → `vscode/resources/win32/inno-big/small-*.bmp` (VSCodium) ; `SetupIconFile`=code.ico **déjà Arclen ✅** ; source `icons/build_icons.sh:40,46` | ~~élevé~~ | ✅ **non-issue 2026-06-04** — BMP wizard déjà logo corail Arclen (vérifié visuellement) |

> Où ça tourne : installeur produit par `build/windows/package.sh` → `gulpfile.vscode.win32.ts` → ISCC, déclenché par **CI** (`.github/workflows/ci-build-windows.yml`) — c'est ainsi que `dist-arclen/` a été fait. Le fix atterrit en source puis re-package (CI ou packaging local). Effort total ~demi-journée, les images wizard étant le plus long.

### Audit source complet (2026-06-01) — autres fuites au-delà de l'installeur

> Scanner `check-brand-leaks.sh --strict` = **0 fuite HIGH** dans le renderer/l10n (les ~130 MEDIUM = liens de doc `aka.ms`/`code.visualstudio.com` dans le *contenu* d'aide → légitimes). **Cause racine** : `apply_branding.sh` ne paramètre QUE les `setpath "product"` ; les seds de packaging restent en dur « VSCodium ». **Fix propre = étendre `apply_branding.sh`** (PAS patcher `prepare_vscode.sh` à la main). ⚠️ **Arclen ne cible que Windows x64** (pas de build Linux/macOS/serveur) → les fuites Linux/macOS/PWA **ne shippent pas** aujourd'hui.

| # | Fuite | file:line | Ship Win desktop ? | Priorité |
|---|---|---|---|---|
| IN0 | **Fix racine** : étendre `apply_branding.sh` pour rebrander code.iss + manifests + copyright via les vars `BRAND_*` | `apply_branding.sh` | — | fait toute la passe d'un coup |
| IN4 | Copyright Electron « VSCodium » (propriétés .exe / About) | `prepare_vscode.sh:322-323` (sed sur `build/lib/electron.ts`) | **OUI** (faible visibilité) | **moyenne** — seul vrai résidu Windows visible hors installeur |
| IN5 | Manifest serveur/PWA « VSCodium » | `prepare_vscode.sh:310-314` | non (web/serveur, non buildé) | basse |
| IN6 | Métadonnées paquets Linux (control/appdata/spec) | `prepare_vscode.sh:337-354` | non (pas de build Linux) | basse |
| IN7 | `package.json` maintainer + défauts `utils.sh` (`APP_NAME:-VSCodium`…) | `prepare_vscode.sh:305`, `utils.sh:3-9` | non (dev / fallback) | basse (hygiène/sécurité) |
| IN8 | URLs sourcemap + policy macOS dans des patches | `patches/00-build-update-sourcemap-url.patch`, `21-policy-use-custom-lib.patch` | non (dev / macOS) | basse |
...
- 🟡 **AD1** Niv.0 : extension officielle **bundlée** ✅ (product.json `builtInExtensions` + vsix Open VSX). **Position recadrée (2026-05-30, décision user — l'autre IA l'avait mise à droite, rejeté)** : PAS de chat à droite auto. `"claudeCode.preferredLocation":"panel"` (product.json `configurationDefaults`, **pas de patch**) donne exactement le flux voulu : (1) **aucun auto-open** au démarrage (ni droite, ni centre), (2) **icône ✳ rail GAUCHE** = launcher de sessions (New session / Local·Web / liste), (3) ouvrir une session = **onglet au CENTRE** (zone éditeur). Vérifié live profil neuf (palette « Claude Code: Open in Editor » → centre). Alt dispo : `claudeCode.useTerminal` (Claude comme terminal, hint « Prefer the Terminal experience »). **2026-06-04** : (a) **walkthrough « Get started » masqué** ✅ — patch `arclen-hide-claude-walkthrough` (filtre les walkthroughs d'extension par préfixe d'id `anthropic.claude-code#` dans `gettingStartedService.ts`, **case-insensitive** : le manifeste capitalise le publisher « Anthropic » → `identifier.value` = `Anthropic.claude-code#…`, un `startsWith` sensible à la casse aurait été un no-op silencieux) ; **live-confirmé** avec Claude chargé = picker walkthrough vide (contrôle positif). (b) **vsix bumpé 2.1.157→2.1.162** (Open VSX, sha256 ✓), live-confirmé (charge, ouvre au centre, commandes OK). (c) **auto-update activé** : `builtInExtensionsEnabledWithAutoUpdates += anthropic.claude-code` → Claude suit Open VSX sans rouvrir la marketplace (`extensionsWorkbenchService.ts` bloque l'update d'un built-in stable sauf s'il est listé) ; le vsix bundlé = plancher/offline, fini les bumps manuels.
...
14. ✅ **Session 2026-06-01** (build packagé local `-s` → copié dans `dist-arclen/Arclen-portable`, vérifié live sur le `.exe` frais) : (a) **Couleurs granulaires** — nouveaux tokens `$focus #2F6E76` + `$cursor #44919A` (halo de focus + curseurs terminal/éditeur atténués ; accent de marque `#5AB8C4` gardé vif sur boutons/badges/onglet actif). (b) **Footer git** (branche+sync) retiré — patch `arclen-hide-git-statusbar` (voir S1). (c) **« Open Editors » → « Sessions & Documents »** — patch `arclen-rename-open-editors` (`openEditorsView.ts` NAME+aria ; vue gardée, juste le libellé, à la demande du user). (d) **Command Palette filtrée** — patch `arclen-descare-command-palette` (`commandsQuickAccess.ts:getGlobalCommandPicks`, blocklist de catégories : developer/debug/task/test/notebook/snippet/emmet/typescript/javascript/references/merge/ports ; listing only, keybindings+API intacts ; ⚠️ masque aussi Reload Window / Toggle Dev Tools → à affiner). (e) **Extensions RÉOUVERTES** (réversible — voir A5/G3). **Installeur VSCodium** → backlog §9. **Audit fuites branding source-level** lancé (suite).
