...
- [ ] **`dev/ci-preflight.sh` — static check for the CI-only build traps (the local build is a false-green for them)** — 2026-06-04: cutting the first daily-driver installer cost 3 failed CI runs (~2 round-trips × 55 min) discovering CI-only failures serially that the local build can't surface: (1) the npm-ci warm-build guard skipped `npm ci` on a CI cache-hit because it only checked root `node_modules`, not `build/node_modules` (where `ternary-stream` lives) → `vscode-min-prepack` "Cannot find package 'ternary-stream'" (fixed 282f45f); (2) `code.iss OutputBaseFilename=ArclenSetup` desynced `prepare_assets.sh`'s `mv VSCodeSetup.exe` → "Prepare assets" failed (fixed f51109b). Compounding: CI runs on every non-`.md` push but **nobody watches it** (local-only philosophy) — it had sat RED ~3 pushes/hours unnoticed. A preflight asserting the known CI-only invariants (guard requires build/node_modules; `code.iss OutputBaseFilename` == the literal `prepare_assets.sh` mv expects; asset-name contracts) before a 55-min dispatch — plus a `gh run list` habit before assuming green — would've caught both in seconds. — mechanism: script + skill note — effort: M
...
## Done
- [x] **Two CI-only build bugs fixed while cutting the first installer (2026-06-04)** — both passed locally, failed only on the CI runner (clean machine / fresh node_modules / the Inno asset step local never runs). **(1) npm-ci guard** now also requires `-d build/node_modules` (`prepare_vscode.sh`, 282f45f): the warm-build skip trusted root `node_modules` + the lock-sha stamp, but build deps (`ternary-stream` &c, imported by `build/lib/*.ts`) live in `build/node_modules`, which the CI cache (`vscode/node_modules` only) doesn't restore → skip-on-cache-hit broke `min-prepack` every push. This is the "first real build validates the npm-ci skip end-to-end" the 2026-05-31 warm-build item promised — it found the gap. **(2) `code.iss OutputBaseFilename` reverted** (`prepare_vscode.sh`, f51109b): the final installer name comes from `prepare_assets.sh`'s `mv VSCodeSetup.exe → ${APP_NAME}Setup-…exe`, so Inno must keep emitting `VSCodeSetup.exe`; renaming it broke the mv ("cannot stat VSCodeSetup.exe", both arches). Full forward-looking detail in `arclen-dev` skill « CI build gotchas ». First green installer = CI run 26960248274 (ArclenSetup-x64-1.121.03735.exe, Claude 2.1.162 bundled, auto-update allowlisted). — mechanism: 2× prepare_vscode.sh edit — effort: S
