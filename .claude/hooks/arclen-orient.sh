...
MESSAGE="ARCLEN IDE — Auto-Orientation (SessionStart hook)
...
- Git Bash: C:\\Users\\AdrianTurion\\AppData\\Local\\Programs\\Git\\bin\\bash.exe (user-level).
...
DEEP REFERENCE: .claude/skills/arclen-ide/SKILL.md  (architecture, patches, branding, troubleshooting).
DEV ORIENTATION (long form): .claude/skills/hook-development/SKILL.md  (note: misnamed historically, slated for rename to arclen-orient — this hook supersedes it for auto-loading)."
